...
### ⚠️ Important Notice for v8 Users
...
- The file is automatically generated and stored at: `~/.config/uxplay-control/uxplayrc`
- Please **do not edit this file manually**, as the extension will overwrite your changes whenever you adjust settings via the UI.
- All existing settings from v7 have been automatically migrated.

### Autostart

You can make UXPlay start automatically at login without keeping the extension panel menu open. Open the **General** preferences page and enable **Start UXPlay on login**. The extension then writes a standard XDG autostart entry to:

```
~/.config/autostart/uxplay-control.desktop
```

The entry uses the same `~/.config/uxplay-control/uxplayrc` that the extension manages, so any settings you change in Preferences apply to the auto-launched instance. You can also configure a startup delay (in seconds) on the same page, useful if your network or audio services need a moment to come up.

To disable the autostart entirely, either toggle the switch off in Preferences or delete the `.desktop` file. Disabling the extension itself does **not** remove the autostart entry — your choice to autostart UXPlay is preserved.
...
4. Create a pull request if you want to contribute your changes.
